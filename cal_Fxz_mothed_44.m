function [Fxz,Gxz] = cal_Fxz_mothed_44(mod, gobs, nodex, nodez)
% 初始化输出矩阵
...
for jj = 1:num_points
    ...
    for jjj = 1:NOTEZ
        for iii = 1:NOTEX
            dis_x(jjj, iii) = nodex(jjj, iii) - xx;
            dis_z(jjj, iii) = nodez(jjj, iii) - zz;
            
            % 奇点判断
            if abs(dis_x(jjj, iii)) < eps_val && abs(dis_z(jjj, iii)) < eps_val
                node_ln(jjj, iii) = 0.0;
                continue;
            end
            
            % 计算ln值
            node_ln(jjj, iii) = log(dis_x(jjj, iii)^2 + dis_z(jjj, iii)^2);
        end
    end
            ...
            if is_level
                ...
                for jjj = 1:NOTEZ
                    for iii = 1:NOTEX
                        ...
                        % 奇点判断
                        if abs(dis_x(jjj, iii)) < eps_val && abs(dis_z(jjj, iii)) < eps_val
                            node_atan(jjj, iii) = 0.0;
                            continue;
                        end
                        
                        % 计算atan2值（注意MATLAB的atan2参数顺序：y, x）
                        node_atan(jjj, iii) = atan2(dis_z(jjj, iii), dis_x(jjj, iii));
                    end
                end

function [Fz,Gz] = cal_Fz_mothed_3(mod, gobs, nodex, nodez)
...
notez = grid_rows + 1;  % Number of horizontal edges (NZ+1)
notex = grid_cols + 1;  % Number of vertical edges (NX+1)
...
for k = 1:size(gobs, 1) % Loop over observation points
    ...
    % 1. Compute contributions of all horizontal edges (top and bottom edges)
    for j = 1:notez  % j: 1..NZ+1
        for i = 1:grid_cols  % i: 1..NX
            ...
            if abs(dz_edge) < dz_eps
                % Horizontal edge formula (corrected sign)
                c1 = atan2(z1, x1);
                c2 = atan2(z2, x2);
                ...
                % Compute horizontal edge contribution
    ...
    % 2. Compute contributions of all vertical edges (left and right edges)
    ...
    % 3. Combine contributions of all grid cells (corrected indexing)
    n = 1; % Grid index
    for i = 1:grid_cols % Grid column loop (1..NX)
        for j = 1:grid_rows  % Grid row loop (1..NZ)
            % Combine the contributions of four edges:
            %   Top edge:    H_edge(j, i)

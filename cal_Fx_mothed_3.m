function [Fx,Gx] = cal_Fx_mothed_3(mod, gobs, nodex, nodez)
...
for k = 1:size(gobs, 1) % Loop over observation points
    ...
    for j = 1:notez  % j: 1..NZ+1
        for i = 1:grid_cols  % i: 1..NX
            ...
            if abs(dz_edge) < dz_eps
                ...
                % General case formulation (consistent with the original code)
                c1 = atan2(z1, x1);
                c2 = atan2(z2, x2);
    ...
    for j = 1:grid_rows  % j: 1..NZ
        for i = 1:notex  % i: 1..NX+1
            ...
            % Vertical edge formulation (using x1 as the reference point)
            c1 = atan2(z1, x1);
            c2 = atan2(z2, x2);
            if c1 < 0, c1 = c1 + 2*pi; end
            if c2 < 0, c2 = c2 + 2*pi; end
